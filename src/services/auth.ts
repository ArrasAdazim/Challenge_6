...
class AuthService {
  ...
  static async registerAdmin(req: RegisterRequest): Promise<User | ErrorResponse> {
    try {
      // Check if email is exist
      const user = await UsersRepository.getUserByEmail(req.email);
      ...
      // if (req.role.length < 6 && req.role.length > 1)
      //   throw new Error("Minimal 1 kata dan Maksimal 6 kata");


      // Encrypt password
      const encryptedPassword = bcrypt.hashSync(req.password, SALT_ROUND);
      // const lowercaseRole = req.role.toLowerCase();
